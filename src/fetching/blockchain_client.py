...
class BlockchainClient:
    ...
    def _validate_connections(self) -> None:
        """Validate connections to blockchain nodes"""
        for chain, web3 in self._clients.items():
            if not web3.is_connected():
                logger.error(f"Failed to connect to {chain.value} node")
                raise ConnectionError(f"Cannot connect to {chain.value} node")
